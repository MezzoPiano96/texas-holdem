...
- 断线自动重连（`sessionStorage` 令牌），真人超时不操作自动弃牌/过牌，不会卡住整桌
...
| 联机协议 | WebSocket，服务器持有唯一权威的游戏状态机；座位号（持久、可能有空隙）与引擎内部下标（压缩、连续）之间维护一层翻译映射；每个客户端收到为其定制的状态快照（自己的手牌可见，其他人的不可见） |
| 国际化 | 运行时语言切换，日志采用"存结构化数据、渲染时按当前语言格式化"的设计，因此切换语言会连历史记录一起重新翻译，而不是只对新内容生效 |
| PWA | `manifest.json` + Service Worker 离线缓存优先策略，图标走 `sips` 批量生成多尺寸 |
...
node server.js     # 启动后会打印局域网内的访问地址
```
...
- **实时系统**：WebSocket 双向通信、服务器权威状态、断线重连、超时容错、并发操作的来源校验（防止串座位）
